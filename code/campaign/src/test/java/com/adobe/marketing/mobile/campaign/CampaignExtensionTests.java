...
public class CampaignExtensionTests {
    ...
    private static final String MESSAGES_CACHE =
            CampaignConstants.CACHE_BASE_DIR
                    ...
                    + File.separator;
    private static final String expectedRulesDownloadUrl =
            "https://testMcias/testServer/testPropertyId/testExperienceCloudId/rules.zip";
    ...
    @Mock DataQueuing mockDataQueueService;
    ...
    @Mock PersistentHitQueue mockPersistentHitQueue;
    @Mock LaunchRulesEngine mockRulesEngine;
    @Mock CampaignRulesDownloader mockCampaignRulesDownloader;
    @Mock Evaluable mockEvaluable;
    @Mock CampaignState mockCampaignState;
    @Mock Application mockApplication;
    @Mock Context mockContext;
    @Mock SharedPreferences mockSharedPreferences;
    @Mock CampaignMessage mockCampaignMessage;
    ...
    public void setup() {
        ...
        campaignExtension =
                new CampaignExtension(
                        mockExtensionApi,
                        mockPersistentHitQueue,
                        mockDataStoreService,
                        mockRulesEngine,
                        mockCampaignState,
                        mockCacheService,
                        mockCampaignRulesDownloader);
    }
    ...
    private SharedStateResult getConfigurationEventData(Map<String, Object> customConfig) {
        ...
        configData.put(
                CampaignConstants.EventDataKeys.STATE_OWNER,
                CampaignConstants.EventDataKeys.Configuration.EXTENSION_NAME);
        configData.put(
                CampaignConstants.EventDataKeys.Configuration.CAMPAIGN_SERVER_KEY, "testServer");
        configData.put(CampaignConstants.EventDataKeys.Configuration.CAMPAIGN_PKEY_KEY, "testPkey");
        configData.put(
                CampaignConstants.EventDataKeys.Configuration.CAMPAIGN_MCIAS_KEY, "testMcias");
        ...
        configData.put(
                CampaignConstants.EventDataKeys.Configuration.GLOBAL_CONFIG_PRIVACY, "optedin");
        configData.put(
                CampaignConstants.EventDataKeys.Configuration.CAMPAIGN_REGISTRATION_DELAY_KEY, 30);
        configData.put(
                CampaignConstants.EventDataKeys.Configuration.CAMPAIGN_REGISTRATION_PAUSED_KEY,
                false);
    ...
    private SharedStateResult getIdentityEventData() {
        ...
        identityData.put(
                CampaignConstants.EventDataKeys.Identity.VISITOR_ID_MID, "testExperienceCloudId");
        SharedStateResult sharedStateResult =
                new SharedStateResult(SharedStateStatus.SET, identityData);
    ...
    private Map<String, Object> getLifecycleEventData() {
        ...
        lifecycleEventData.put(
                CampaignConstants.EventDataKeys.Lifecycle.LIFECYCLE_CONTEXT_DATA, lifecycleData);
    ...
    private Map<String, Object> getMessageConsequenceEventData(RuleConsequence consequence) {
        ...
        triggeredConsequenceData.put(
                CampaignConstants.EventDataKeys.RuleEngine.CONSEQUENCE_TRIGGERED, consequence);
        return triggeredConsequenceData;
    }

    private Map<String, Object> getMessageTrackEventData(
            String broadlogId, String deliveryId, String action) {
        ...
        setupServiceProviderMockAndRunTest(
                () -> {
                    when(mockExtensionApi.getSharedState(
                            ...
                            .thenReturn(getConfigurationEventData(new HashMap<>()));
                    when(mockExtensionApi.getSharedState(
                                    eq("com.adobe.module.identity"),
                                    any(Event.class),
                                    anyBoolean(),
                                    any(SharedStateResolution.class)))
                            .thenReturn(getIdentityEventData());
                    campaignExtension =
                            new CampaignExtension(
                                    mockExtensionApi,
                                    mockPersistentHitQueue,
                    ...
                    Event testEvent =
                            new Event.Builder(
                                    ...
                                    .setEventData(
                                            getConfigurationEventData(new HashMap<>()).getValue())
                                    .build();
    ...
    @Test
    public void
            test_readyForEvent_when_identityEventReceived_and_configurationAndIdentitySharedStateDataPresent_then_rulesDownloadTriggered_and_readyForEventIsTrue() {
        // setup
        setupServiceProviderMockAndRunTest(
                () -> {
                    when(mockExtensionApi.getSharedState(
                    ...
                    // verify
                    assertTrue(campaignExtension.readyForEvent(testEvent));
                    verify(mockCampaignRulesDownloader, times(1))
                            .loadRulesFromUrl(eq(expectedRulesDownloadUrl), eq(null));
                });
    }
    ...
    public void test_handleWildcardEvents_happy() {
        ...
        setupServiceProviderMockAndRunTest(
                () -> {
                    ...
                    List<RuleConsequence> ruleConsequenceList =
                            new ArrayList<RuleConsequence>() {
                                {
    ...
    @Test
    public void
            test_handleRuleEngineResponseEvents_when_validConsequenceForLocalNotification_happy() {
        // setup
        try (MockedStatic<CampaignMessage> campaignMessageMockedStatic =
                Mockito.mockStatic(CampaignMessage.class)) {
            campaignMessageMockedStatic
                    .when(() -> CampaignMessage.createMessageObject(any(), any()))
    ...
    @Test
    public void
            test_handleRuleEngineResponseEvents_when_nullDetailsPresentInConsequence_then_shouldNotShowMessage() {
        // setup
        try (MockedStatic<CampaignMessage> campaignMessageMockedStatic =
                Mockito.mockStatic(CampaignMessage.class)) {
            campaignMessageMockedStatic
    ...
    @Test
    public void
            test_handleRuleEngineResponseEvents_when_detailsMissingTemplate_then_shouldNotShowMessage() {
        // setup
        try (MockedStatic<CampaignMessage> campaignMessageMockedStatic =
                Mockito.mockStatic(CampaignMessage.class)) {
            campaignMessageMockedStatic
    ...
    public void test_setCampaignState_when_nullStateOwner_then_shouldDoNothing() {
        ...
        // verify
        verify(mockCampaignState, times(0))
                .setState(any(SharedStateResult.class), any(SharedStateResult.class));
    }
    ...
    public void test_setCampaignState_when_configurationStateOwner_then_shouldSetCampaignState() {
        ...
        Event testEvent =
                new Event.Builder(
                                "Test event", EventType.CONFIGURATION, EventSource.RESPONSE_CONTENT)
                        .setEventData(getConfigurationEventData(new HashMap<>()).getValue())
                        .build();
        when(mockExtensionApi.getSharedState(
                        eq(CampaignConstants.EventDataKeys.Configuration.EXTENSION_NAME),
                        any(Event.class),
                        anyBoolean(),
                        any(SharedStateResolution.class)))
                .thenReturn(getConfigurationEventData(new HashMap<>()));
        ...
        // verify setState called with the first argument being present. first argument is a config
        // SharedStateResult.
    ...
    public void test_setCampaignState_when_identityStateOwner_then_shouldSetCampaignState() {
        // setup
        Event testEvent =
                new Event.Builder("Test event", EventType.IDENTITY, EventSource.RESPONSE_CONTENT)
                        .setEventData(getConfigurationEventData(new HashMap<>()).getValue())
                        .build();
        when(mockExtensionApi.getSharedState(
                        eq(CampaignConstants.EventDataKeys.Identity.EXTENSION_NAME),
                        any(Event.class),
                        anyBoolean(),
                        any(SharedStateResolution.class)))
                .thenReturn(getIdentityEventData());
        ...
        // verify setState called with the second argument being present. second argument is a
        // identity SharedStateResult.
    ...
    public void test_handleLinkageFieldsEvent_when_eventIsNull() {
        try (MockedStatic<Log> logMockedStatic = Mockito.mockStatic(Log.class)) {
            ...
            verify(mockCacheService, times(0))
                    .remove(eq(CampaignConstants.RULES_CACHE_FOLDER), eq(""));
            verify(mockCampaignRulesDownloader, times(0))
                    .loadRulesFromUrl(anyString(), anyString());
        }
    }
    ...
    public void test_handleLinkageFieldsEvent_when_eventDataIsEmpty() {
        try (MockedStatic<Log> logMockedStatic = Mockito.mockStatic(Log.class)) {
            ...
            Event testEvent =
                    new Event.Builder(
                                    "Test event", EventType.CAMPAIGN, EventSource.REQUEST_IDENTITY)
                            .build();
    ...
    public void test_handleLinkageFieldsEvent_setValidLinkageFields() {
        ...
        try (MockedStatic<Base64> ignored = Mockito.mockStatic(Base64.class);
            ...
            campaignState.setState(
                    getConfigurationEventData(new HashMap<>()), getIdentityEventData());
            campaignExtension =
                    new CampaignExtension(
                            mockExtensionApi,
                            mockPersistentHitQueue,
            ...
            verify(mockCampaignRulesDownloader, times(1))
                    .loadRulesFromUrl(eq(expectedRulesDownloadUrl), eq(encodedLinkageFields));
        }
    }
    ...
    public void test_handleLinkageFieldsEvent_setValidLinkageFields_when_campaignStateNotReady() {
        ...
        try (MockedStatic<Base64> ignored = Mockito.mockStatic(Base64.class)) {
            ...
            verify(mockCacheService, times(0))
                    .remove(
                            eq(CampaignConstants.CACHE_BASE_DIR),
                            eq(CampaignConstants.RULES_CACHE_FOLDER));
            verify(mockCampaignRulesDownloader, times(0))
                    .loadRulesFromUrl(anyString(), anyString());
        }
    }
    ...
    public void test_handleResetLinkageFields_happy() {
        ...
        try (MockedStatic<Utils> utilsMockedStatic = Mockito.mockStatic(Utils.class)) {
            ...
            Event testEvent =
                    new Event.Builder("Test event", EventType.CAMPAIGN, EventSource.REQUEST_RESET)
                            .build();
            ...
            utilsMockedStatic.verify(() -> Utils.cleanDirectory(any(File.class)), times(1));
            verify(mockCampaignRulesDownloader, times(1))
                    .loadRulesFromUrl(eq(expectedRulesDownloadUrl), eq(""));
        }
    }
    ...
    public void test_processConfiguration_When_NullEvent() {
        ...
        // verify
        verify(mockPersistentHitQueue, times(0))
                .handlePrivacyChange(any(MobilePrivacyStatus.class));
    ...
    public void test_processConfiguration_When_PrivacyOptedIn() {
        ...
        // verify
        verify(mockPersistentHitQueue, times(1))
                .handlePrivacyChange(eq(MobilePrivacyStatus.OPT_IN));
        verify(mockCampaignRulesDownloader, times(1))
                .loadRulesFromUrl(eq(expectedRulesDownloadUrl), eq(null));
    }
    ...
    public void test_processConfiguration_When_PrivacyOptOut() {
        ...
        try (MockedStatic<Utils> utilsMockedStatic = Mockito.mockStatic(Utils.class)) {
            ...
            // verify
            verify(mockPersistentHitQueue, times(1))
                    .handlePrivacyChange(eq(MobilePrivacyStatus.OPT_OUT));
            String linkageFields = campaignExtension.getLinkageFields();
            assertEquals("", linkageFields);
    ...
    public void test_processConfiguration_When_PrivacyUnknown() {
        ...
        HashMap<String, Object> configData = new HashMap<>();
        configData.put(
                CampaignConstants.EventDataKeys.Configuration.GLOBAL_CONFIG_PRIVACY, "unknown");
        campaignState.setState(getConfigurationEventData(configData), getIdentityEventData());
        campaignExtension =
                new CampaignExtension(
                        mockExtensionApi,
        ...
        // verify
        verify(mockPersistentHitQueue, times(1))
                .handlePrivacyChange(eq(MobilePrivacyStatus.UNKNOWN));
        verify(mockCampaignRulesDownloader, times(0)).loadRulesFromUrl(anyString(), anyString());
    }
    ...
    public void test_processMessageInformation_when_campaignConfigured_then_shouldProcessRequest() {
        ...
        ArgumentCaptor<DataEntity> dataEntityArgumentCaptor =
        ...
        Event testEvent =
                new Event.Builder("Test event", EventType.GENERIC_DATA, EventSource.OS)
                        .setEventData(getMessageTrackEventData("h2347", "bb65", "2"))
                        .build();
    ...
    public void test_processMessageInformation_when_noEventData_then_shouldNotProcessRequest() {
        ...
        Event testEvent =
                new Event.Builder("Test event", EventType.GENERIC_DATA, EventSource.OS).build();
    ...
    @Test
    public void
            test_processMessageInformation_when_campaignNotConfigured_then_shouldNotProcessRequest() {
        // setup
        CampaignState campaignState = new CampaignState();
        campaignExtension =
                new CampaignExtension(
    ...
    public void test_processMessageInformation_when_emptyBroadlogId_then_shouldNotProcessRequest() {
        ...
        Event testEvent =
                new Event.Builder("Test event", EventType.GENERIC_DATA, EventSource.OS)
                        .setEventData(getMessageTrackEventData("", "bb65", "2"))
                        .build();
    ...
    public void test_processMessageInformation_when_nullBroadlogId_then_shouldNotProcessRequest() {
        ...
        Event testEvent =
                new Event.Builder("Test event", EventType.GENERIC_DATA, EventSource.OS)
                        .setEventData(getMessageTrackEventData(null, "bb65", "2"))
                        .build();
    ...
    public void test_processMessageInformation_when_emptyDeliveryId_then_shouldNotProcessRequest() {
        ...
        Event testEvent =
                new Event.Builder("Test event", EventType.GENERIC_DATA, EventSource.OS)
                        .setEventData(getMessageTrackEventData("h2347", "", "2"))
                        .build();
    ...
    public void test_processMessageInformation_when_nullDeliveryId_then_shouldNotProcessRequest() {
        ...
        Event testEvent =
                new Event.Builder("Test event", EventType.GENERIC_DATA, EventSource.OS)
                        .setEventData(getMessageTrackEventData("h2347", null, "2"))
                        .build();
    ...
    public void test_processMessageInformation_when_emptyAction_then_shouldNotProcessRequest() {
        ...
        Event testEvent =
                new Event.Builder("Test event", EventType.GENERIC_DATA, EventSource.OS)
                        .setEventData(getMessageTrackEventData("h2347", "bb65", ""))
                        .build();
    ...
    public void test_processMessageInformation_when_nullAction_then_shouldNotProcessRequest() {
        ...
        Event testEvent =
                new Event.Builder("Test event", EventType.GENERIC_DATA, EventSource.OS)
                        .setEventData(getMessageTrackEventData("h2347", "bb65", null))
                        .build();
    ...
    public void test_processMessageInformation_when_PrivacyOptOut_then_shouldNotProcessRequest() {
        ...
        Map<String, Object> configData = new HashMap<>();
        configData.put(
                CampaignConstants.EventDataKeys.Configuration.GLOBAL_CONFIG_PRIVACY, "optedout");
    ...
    public void test_processLifecycleUpdate_when_EventIsEmpty() {
        // setup
        Event testEvent =
                new Event.Builder("Test event", EventType.LIFECYCLE, EventSource.RESPONSE_CONTENT)
                        .build();
    ...
    public void test_processLifecycleUpdate_when_campaignConfigured_then_shouldQueueHit() {
        ...
        setupServiceProviderMockAndRunTest(
                () -> {
                    ...
                    when(mockDataStoreService.getNamedCollection(anyString()))
                            .thenReturn(fakeNamedCollection);
                    campaignExtension =
                            new CampaignExtension(
                                    mockExtensionApi,
                                    mockPersistentHitQueue,
                    ...
                    Event testEvent =
                            new Event.Builder(
                                            ...
                                            EventType.LIFECYCLE,
                                            EventSource.RESPONSE_CONTENT)
                                    .setEventData(getLifecycleEventData())
                                    .build();

                    String url =
                            String.format(
                                    CampaignConstants.CAMPAIGN_REGISTRATION_URL,
                                    campaignState.getCampaignServer(),
                    ...
                    // verify
                    verify(mockPersistentHitQueue, times(1))
                            .queue(dataEntityArgumentCaptor.capture());
                    DataEntity capturedDataEntity = dataEntityArgumentCaptor.getValue();
                    CampaignHit campaignHit = Utils.campaignHitFromDataEntity(capturedDataEntity);
                    assertEquals(url, campaignHit.url);
                    assertTrue(campaignHit.payload.contains("\"pushPlatform\":\"gcm\""));
                    assertTrue(
                            campaignHit.payload.contains(
                                    "\"marketingCloudId\":\""
                                            + campaignState.getExperienceCloudId()
                                            + "\""));
                });
    ...
    public void
        ...
        Map<String, Object> configData = new HashMap<>();
        configData.put(
                CampaignConstants.EventDataKeys.Configuration.CAMPAIGN_REGISTRATION_DELAY_KEY, 0);
        CampaignState campaignState = new CampaignState();
        campaignState.setState(getConfigurationEventData(new HashMap<>()), getIdentityEventData());
        campaignExtension =
                new CampaignExtension(
    ...
    @Test
    public void
            test_processLifecycleUpdate_when_registrationPausedIsEqualToFalse_then_shouldQueueHit() {
        // setup
        // set registration delay to 30 days
        ArgumentCaptor<DataEntity> dataEntityArgumentCaptor =
                ArgumentCaptor.forClass(DataEntity.class);
    ...
    @Test
    public void
            test_processLifecycleUpdate_when_registrationPausedIsEqualToTrue_then_shouldNotQueueHit() {
        // setup
        // set registration delay to 30 days
        FakeNamedCollection fakeNamedCollection = new FakeNamedCollection();
        when(mockDataStoreService.getNamedCollection(anyString())).thenReturn(fakeNamedCollection);
    ...
    public void test_dispatchMessageInfo_happy() {
        ...
        String action = "7";
        expectedGenericData.put(
                CampaignConstants.EventDataKeys.Campaign.TRACK_INFO_KEY_BROADLOG_ID, broadlogId);
        expectedGenericData.put(
                CampaignConstants.EventDataKeys.Campaign.TRACK_INFO_KEY_DELIVERY_ID, deliveryId);
        expectedGenericData.put(
                CampaignConstants.EventDataKeys.Campaign.TRACK_INFO_KEY_ACTION, action);
                ...
                () -> {
                    try (MockedStatic<Utils> utilsMockedStatic = Mockito.mockStatic(Utils.class)) {
                        ...
                        utilsMockedStatic.verify(
                                () -> Utils.cleanDirectory(any(File.class)), times(1));
                    }
                });
    }
    ...
    public void testACPDatastoreMigratedToAEPNamedCollection() throws Exception {
        ...
        when(mockDataStoreService.getNamedCollection(
                        eq(CampaignConstants.CAMPAIGN_NAMED_COLLECTION_NAME)))
                .thenReturn(testNamedCollection);
        final File testDatastore = TestUtils.getResource("CampaignDatastore.xml");
        final File testSharedPrefsFile =
                new File(
                        cacheDir
                                + File.separator
                                + "shared_prefs"
                                + File.separator
                                + "CampaignDatastore.xml");
        ...
        try (MockedStatic<ServiceProvider> serviceProviderMockedStatic =
            ...
            try (MockedStatic<MobileCore> mobileCoreMockedStatic =
                ...
                when(mockSharedPreferences.getString(
                                eq(CampaignConstants.CAMPAIGN_NAMED_COLLECTION_REMOTES_URL_KEY),
                                anyString()))
                        .thenReturn(expectedRulesDownloadUrl);
                when(mockSharedPreferences.getString(
                                eq(
                                        CampaignConstants
                                                .CAMPAIGN_NAMED_COLLECTION_EXPERIENCE_CLOUD_ID_KEY),
                                anyString()))
                        .thenReturn("testEcid");
                when(mockSharedPreferences.getLong(
                                eq(
                                        CampaignConstants
                                                .CAMPAIGN_NAMED_COLLECTION_REGISTRATION_TIMESTAMP_KEY),
                                anyLong()))
                        .thenReturn(1671470288599L);
                ...
                // verify
                assertEquals(
                        expectedRulesDownloadUrl,
                        testNamedCollection.getString(
