...
public class CampaignRulesDownloaderTests {
    ...
    public void setup() {
        ...
        when(mockRuleConsequence.getType())
                .thenReturn(CampaignConstants.MESSAGE_CONSEQUENCE_MESSAGE_TYPE);
    }
    ...
    private void setupServiceProviderMockAndRunTest(
            boolean networkServiceNull, Runnable testRunnable) {
        ...
        try (MockedStatic<ServiceProvider> serviceProviderMockedStatic =
            ...
            when(mockCacheService.get(
                            ...
                            eq(CampaignConstants.RULES_JSON_FILE_NAME)))
    ...
    @Test
    public void
            test_loadRulesFromUrl_When_etagPresentInCachedFileMetadata_Then_RuleDownloadRequestContainsEtagInHeaders() {
        // setup
        ...
        when(mockCacheService.get(
                        eq(
                                CampaignConstants.CACHE_BASE_DIR
                                        + File.separator
                                        + CampaignConstants.RULES_CACHE_FOLDER),
                        eq(CampaignConstants.ZIP_HANDLE)))
                .thenReturn(mockCacheResult);
                ...
                () -> {
                    ...
                    try {
                        when(mockHttpConnection.getInputStream())
                                .thenReturn(new FileInputStream(zipFile));
                    } catch (FileNotFoundException e) {
                        fail(e.getMessage());
                    }
                    doAnswer(
                    ...
                    assertEquals(
                            TIME_SINCE_EPOCH_RFC2882,
                            headers.get(CampaignConstants.HTTP_HEADER_IF_MODIFIED_SINCE));
    ...
    @Test
    public void
            test_loadRulesFromUrl_When_WeakEtagPresentInCachedFileMetadata_Then_RuleDownloadRequestContainsWeakEtagInHeaders() {
        // setup
        ArgumentCaptor<NetworkRequest> networkRequestArgumentCaptor =
                ArgumentCaptor.forClass(NetworkRequest.class);
        // setup cached rules zip
        ...
        setupServiceProviderMockAndRunTest(
                ...
                () -> {
                    ...
                    Map<String, String> headers = capturedNetworkRequest.getHeaders();
                    assertEquals(
                            WEAK_ETAG, headers.get(CampaignConstants.HTTP_HEADER_IF_NONE_MATCH));
                    assertEquals(
                            TIME_SINCE_EPOCH_RFC2882,
                            headers.get(CampaignConstants.HTTP_HEADER_IF_MODIFIED_SINCE));
                    // verify extracted rules json is cached
    ...
    @Test
    public void
            test_loadRulesFromUrl_When_NoCachedMessagesPresent_Then_RuleDownloadRequestContainsNoEtagInHeaders() {
        // setup
        ArgumentCaptor<NetworkRequest> networkRequestArgumentCaptor =
                ArgumentCaptor.forClass(NetworkRequest.class);
        // setup no cached rules zip present
        when(mockCacheService.get(
                        eq(CampaignConstants.CACHE_BASE_DIR), eq(CampaignConstants.ZIP_HANDLE)))
                .thenReturn(null);
    ...
    @Test
    public void
            test_loadRulesFromUrl_When_LinkageFieldsSet_Then_RuleDownloadRequestContainsEncodedLinkageFieldsInHeaders() {
        // setup
        ArgumentCaptor<NetworkRequest> networkRequestArgumentCaptor =
                ArgumentCaptor.forClass(NetworkRequest.class);
        // setup cached rules zip
        ...
        setupServiceProviderMockAndRunTest(
                ...
                () -> {
                    ...
                    assertEquals(
                            linkageFields,
                            headers.get(CampaignConstants.LINKAGE_FIELD_NETWORK_HEADER));
                    // verify extracted rules json is cached
                    verify(mockCacheService, times(1))
                            .set(
                                    eq(
    ...
    @Test
    public void
            test_loadRulesFromUrl_When_RuleDownloadRequestResponseContainsNotModified_Then_RulesLoadedFromCache() {
        // setup
        ArgumentCaptor<NetworkRequest> networkRequestArgumentCaptor =
                ArgumentCaptor.forClass(NetworkRequest.class);
        // setup cached rules zip
    ...
    @Test
    public void
            test_loadRulesFromUrl_When_RuleDownloadRequestResponseContainsNotFound_Then_NoRulesLoadedOrCached() {
        // setup
        ArgumentCaptor<NetworkRequest> networkRequestArgumentCaptor =
                ArgumentCaptor.forClass(NetworkRequest.class);
    ...
    @Test
    public void
            test_loadRulesFromUrl_When_InvalidZipContentDownloaded_Then_NoRulesLoadedOrCached() {
        // setup
        ArgumentCaptor<NetworkRequest> networkRequestArgumentCaptor =
                ArgumentCaptor.forClass(NetworkRequest.class);
    ...
    public void test_cacheRemoteAssets_When_campaignRulesAreEmpty_Then_AssetsNotDownloaded() {
        ...
        try (MockedConstruction mockConstruction =
                mockConstruction(CampaignMessageAssetsDownloader.class)) {
            setupServiceProviderMockAndRunTest(
                    false,
                    () -> {
        ...
        try (MockedConstruction mockConstruction =
            ...
            setupServiceProviderMockAndRunTest(
                    ...
                    () -> {
                        ...
                        // verify
                        CampaignMessageAssetsDownloader mockCampaignMessageAssetsDownloader =
                                (CampaignMessageAssetsDownloader)
                                        mockConstruction.constructed().get(0);
                        verify(mockCampaignMessageAssetsDownloader, times(1))
                                .downloadAssetCollection();
                    });
        }
    }
