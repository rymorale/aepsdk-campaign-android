...
class AlertMessage extends CampaignMessage {
    ...
    /**
     * Serves as the delegate for handling {@code AlertMessage} events triggered by the {@code
     * UIService}.
     */
    class UIAlertMessageUIListener implements AlertEventListener {
         ...
         *
         * <p>Checks to see if the alert message's url is populated and if so adds the url to the
         * context data map then calls {@link #clickedWithData(Map)}. If a url is not present then
         * call the {@link #clickedThrough()} method implemented in the parent class {@link
         * CampaignMessage}.
         */
        @Override
         ...
         *
         * <p>Calls the {@link #viewed()} method implemented in the parent class {@link
         * CampaignMessage}.
         */
        @Override
         ...
         *
         * <p>Calls the {@link #triggered()} method implemented in the parent class {@link
         * CampaignMessage}.
         */
        @Override
        ...
        @Override
        public void onError(
                final @NonNull Presentable<Alert> presentable,
                final @NonNull PresentationError presentationError) {
            Log.debug(
                    CampaignConstants.LOG_TAG,
                    SELF_TAG,
                    "Error occurred when attempting to display the alert message");
        }
    }
     ...
     * @param consequence {@code Map<String, Object>} instance containing {@code CampaignMessage}
     *     defining payload
     * @throws CampaignMessageRequiredFieldMissingException if {@code consequence} is null or if any
     *     required field for an {@link AlertMessage} is null or empty
     */
    ...
    /**
     * Parses a {@code CampaignRuleConsequence} instance defining message payload for an {@code
     * AlertMessage} object.
     *
     * <p>Required fields:
     *
     ...
     *       {@link String} containing the title for this message
     *   <li>{@value
     *       CampaignConstants.EventDataKeys.RuleEngine#MESSAGE_CONSEQUENCE_DETAIL_KEY_CONTENT} -
     *       {@code String} containing the content of the message
     *   <li>{@value
     *       CampaignConstants.EventDataKeys.RuleEngine#MESSAGE_CONSEQUENCE_DETAIL_KEY_CANCEL} -
     *       {@code String} containing the text of the cancel or negative action button on this
     *       message
     * </ul>
     *
     * Optional fields:
     ...
     * <ul>
     *   <li>{@value
     *       CampaignConstants.EventDataKeys.RuleEngine#MESSAGE_CONSEQUENCE_DETAIL_KEY_CONFIRM} -
     *       {@code String} containing the text of the confirm or positive action button on this
     *       message
     *   <li>{@value CampaignConstants.EventDataKeys.RuleEngine#MESSAGE_CONSEQUENCE_DETAIL_KEY_URL}
     *       - {@code String} containing a URL destination to be shown on positive click-through
     * </ul>
     *
     * @param consequence {@code RuleConsequence} instance containing the message payload to be
     *     parsed
     * @throws CampaignMessageRequiredFieldMissingException if any of the required fields are
     *     missing from {@code consequence}
     */
    @SuppressWarnings("unchecked")
    private void parseAlertMessagePayload(final RuleConsequence consequence)
            throws CampaignMessageRequiredFieldMissingException {
        Log.trace(
                CampaignConstants.LOG_TAG,
                SELF_TAG,
                "parseAlertMessagePayload - Parsing rule consequence to show alert message with"
                        + " messageid %s",
                messageId);
        ...
        cancelButtonText =
                DataReader.optString(
                        detailDictionary,
                        CampaignConstants.EventDataKeys.RuleEngine
                                .MESSAGE_CONSEQUENCE_DETAIL_KEY_CANCEL,
                        "");

        if (StringUtils.isNullOrEmpty(cancelButtonText)) {
            throw new CampaignMessageRequiredFieldMissingException(
                    "Alert Message cancel button text is empty.");
        }
        ...
        confirmButtonText =
                DataReader.optString(
                        detailDictionary,
                        CampaignConstants.EventDataKeys.RuleEngine
                                .MESSAGE_CONSEQUENCE_DETAIL_KEY_CONFIRM,
                        "");
        ...
        if (StringUtils.isNullOrEmpty(confirmButtonText)) {
            Log.trace(
                    CampaignConstants.LOG_TAG,
                    SELF_TAG,
                    "Tried to read \"confirm\" for Alert message but found none. This is not a"
                            + " required field.");
        }
        ...
        url =
                DataReader.optString(
                        detailDictionary,
                        CampaignConstants.EventDataKeys.RuleEngine
                                .MESSAGE_CONSEQUENCE_DETAIL_KEY_URL,
                        "");
        ...
        if (StringUtils.isNullOrEmpty(url)) {
            Log.trace(
                    CampaignConstants.LOG_TAG,
                    SELF_TAG,
                    "Tried to read url for Alert message but found none. This is not a required"
                            + " field.");
        }
    }
     ...
     *
     * <p>This method registers a {@link UIAlertMessageUIListener} instance with the {@code
     * UIService} to handle message interaction events.
     *
    ...
    void showMessage() {
        Log.debug(
                CampaignConstants.LOG_TAG,
                SELF_TAG,
                "Attempting to show Alert message with ID %s ",
                messageId);

        final UIAlertMessageUIListener alertListener = new UIAlertMessageUIListener();
        final AlertSettings alertSetting =
                new AlertSettings.Builder()
                        .title(title)
                        .message(content)
                        .positiveButtonText(confirmButtonText)
                        .negativeButtonText(cancelButtonText)
                        .build();
        final Presentable<Alert> alertPresentable =
                uiService.create(
                        new Alert(alertSetting, alertListener),
     ...
     *
     * <p>This method returns false as no assets need to be downloaded for {@code AlertMessage}
     * class.
     *
    ...
    /**
     * Invokes method in parent {@code CampaignMessage} class so that the {@code url} can be shown
     * by the {@code UIService}.
     *
     * @see #openUrl(String)
     */
    void showUrl() {
